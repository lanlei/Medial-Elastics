...
namespace CudaSolver
{
	...
	__global__ void computeDistanceFromSurfacePointToPrimitve
	(uint32_t * dev_surface_points_num, qeal * dev_surface_points_position, uint32_t * dev_total_medial_cones_num, uint32_t * dev_total_medial_slabs_num, uint32_t * dev_surface_points_band_mp_index, qeal * dev_surface_points_band_mp_interpolation, uint32_t * dev_medial_cones, uint32_t * dev_medial_slabs, qeal * dev_medial_nodes, qeal * dev_rest_medial_nodes, qeal * dev_dist_surface_point_to_mp)
	{
		__shared__ uint32_t surface_points_num;
		__shared__ uint32_t medial_cones_num;
		...
		{
			surface_points_num = *dev_surface_points_num;
			medial_cones_num = *dev_total_medial_cones_num;
		}
		__syncthreads();
		...
		{
			qeal p[3], t[3];
			getVector3FromList(tid, p, dev_surface_points_position);
			getVector3FromList(tid, t, dev_surface_points_band_mp_interpolation);
			uint32_t pid = dev_surface_points_band_mp_index[tid];
			if (pid < medial_cones_num)
			{
				uint32_t cone_id = pid;
				uint32_t mid0 = dev_medial_cones[2 * cone_id];
				uint32_t mid1 = dev_medial_cones[2 * cone_id + 1];

				qeal m0[3], m1[3], r0, r1;
				m0[0] = dev_medial_nodes[4 * mid0];
				m0[1] = dev_medial_nodes[4 * mid0 + 1];
				m0[2] = dev_medial_nodes[4 * mid0 + 2];
				r0 = dev_rest_medial_nodes[4 * mid0 + 3];
				m1[0] = dev_medial_nodes[4 * mid1];
				m1[1] = dev_medial_nodes[4 * mid1 + 1];
				m1[2] = dev_medial_nodes[4 * mid1 + 2];
				r1 = dev_rest_medial_nodes[4 * mid1 + 3];

				p[0] = p[0] - (t[0] * m0[0] + t[1] * m1[0]);
				p[1] = p[1] - (t[0] * m0[1] + t[1] * m1[1]);
				p[2] = p[2] - (t[0] * m0[2] + t[1] * m1[2]);

				dev_dist_surface_point_to_mp[tid] = getVectorNorm(p) - (t[0] * r0 + t[1] * r1);

			}
			else
			{
				uint32_t slab_id = pid - medial_cones_num;
				uint32_t mid0 = dev_medial_slabs[3 * slab_id];
				uint32_t mid1 = dev_medial_slabs[3 * slab_id + 1];
				uint32_t mid2 = dev_medial_slabs[3 * slab_id + 2];

				qeal m0[3], m1[3], m2[3], r0, r1, r2;
				m0[0] = dev_medial_nodes[4 * mid0];
				m0[1] = dev_medial_nodes[4 * mid0 + 1];
				m0[2] = dev_medial_nodes[4 * mid0 + 2];
				r0 = dev_rest_medial_nodes[4 * mid0 + 3];
				...
				m1[2] = dev_medial_nodes[4 * mid1 + 2];
				r1 = dev_rest_medial_nodes[4 * mid1 + 3];
				m2[0] = dev_medial_nodes[4 * mid2];
				m2[1] = dev_medial_nodes[4 * mid2 + 1];
				m2[2] = dev_medial_nodes[4 * mid2 + 2];
				r2 = dev_rest_medial_nodes[4 * mid2 + 3];

				p[0] = p[0] - (t[0] * m0[0] + t[1] * m1[0] + t[2] * m2[0]);
				p[1] = p[1] - (t[0] * m0[1] + t[1] * m1[1] + t[2] * m2[1]);
				p[2] = p[2] - (t[0] * m0[2] + t[1] * m1[2] + t[2] * m2[2]);

				dev_dist_surface_point_to_mp[tid] = getVectorNorm(p) - (t[0] * r0 + t[1] * r1 + t[2] * r2);
			}
		}

	}

	__global__ void exactBoundingPrimitive
	(
		uint32_t* dev_total_medial_cones_num,
		uint32_t* dev_total_medial_slabs_num,
	...
	{
		__shared__ uint32_t medial_primitive_num;

		const int length = gridDim.x *  blockDim.x;
		int tid = (blockIdx.x  * blockDim.x) + threadIdx.x;
		if (threadIdx.x == 0)
		{
			medial_primitive_num = *dev_total_medial_cones_num + *dev_total_medial_slabs_num;
		}
		__syncthreads();

		for (; tid < medial_primitive_num; tid += length)
		{
			uint32_t num = dev_mp_enclosed_surface_points_num[tid];
			uint32_t offset = dev_mp_enclosed_surface_points_offset[tid];
			qeal scale = 0.0;
			for (uint32_t i = 0; i < num; i++)
			{
				uint32_t point_id = dev_mp_enclosed_surface_points_list[offset + i];
				qeal s = dev_dist_surface_point_to_mp[point_id];
				if (scale < s)
					scale = s;
			}
			if(dev_enlarge_primitives[tid] < scale)
				dev_enlarge_primitives[tid] = scale * 2.0;
		}
	}

	__host__ void exactBoundingPrimitiveHost
	(
		uint32_t * host_surface_points_num,
		uint32_t * dev_surface_points_num,
		qeal * dev_surface_points_position,
	...
	)
	{
		...
		computeDistanceFromSurfacePointToPrimitve << <gridSize, blockSize >> >
			(
				dev_surface_points_num, 
				dev_surface_points_position, 
				dev_total_medial_cones_num, 
				dev_total_medial_slabs_num, 
				dev_surface_points_band_mp_index, 
				dev_surface_points_band_mp_interpolation, 
				dev_medial_cones, 
				dev_medial_slabs, 
				dev_medial_nodes, 
				dev_rest_medial_nodes, 
				dev_dist_surface_point_to_mp
				);
		cudaDeviceSynchronize();

		num_block = ((*host_total_medial_cones_num + *host_total_medial_slabs_num) + (THREADS_NUM - 1)) / THREADS_NUM;
		gridSize = dim3(num_block);

		exactBoundingPrimitive << <gridSize, blockSize >> >
			(
				dev_total_medial_cones_num,
				dev_total_medial_slabs_num,
				dev_mp_enclosed_surface_points_list,
				dev_mp_enclosed_surface_points_offset,
				dev_mp_enclosed_surface_points_num,
				dev_dist_surface_point_to_mp,
				dev_enlarge_primitives
			);
		cudaDeviceSynchronize();
