#include "BendingCactusSimulator.h"
...
namespace MECR
{
	bool BendingCactusSimulator::loadSceneFromConfig(const std::string filename, TiXmlElement* header_item)
	{
		...
		if (header_item && item_name == std::string("simulator"))
		{
			...
			while (sub_item)
			{
				...
				{
					BendingCactusConfig* mc = new BendingCactusConfig();
	...
	bool BendingCactusSimulator::loadSceneFromBinary(std::ifstream& fb)
	{
		uint32_t size;
	...
	void BendingCactusSimulator::saveSceneAsBinary(std::ofstream& fb)
	{
		uint32_t size;
	...
	void BendingCactusSimulator::initSimulator()
	{
	...
	void BendingCactusSimulator::transferToCudaBuffer()
	{
		...
		cudaMalloc((void**)&dev_tet_nodes_velocity, sizeof(qeal) * host_total_fullspace_dim);
		...
		{
			cudaMalloc((char**)&(dev_dnInfo[i]), sizeof(uint32_t));
			mMatrixX sys = host_sys[i];
			cudaMalloc((char**)&(dev_sys[i]), sizeof(qeal) * sys.size());
			cudaMemcpy(dev_sys[i], sys.data(), sizeof(qeal) * sys.size(), cudaMemcpyHostToDevice);

			mMatrixX t = host_sys_project_matrix_x[i];
			cudaMalloc((char**)&(dev_sys_project_matrix_x[i]), sizeof(qeal) * t.size());
			cudaMemcpy(dev_sys_project_matrix_x[i], t.data(), sizeof(qeal) * t.size(), cudaMemcpyHostToDevice);

			t = host_sys_project_matrix_y[i];
			cudaMalloc((char**)&(dev_sys_project_matrix_y[i]), sizeof(qeal) * t.size());
			cudaMemcpy(dev_sys_project_matrix_y[i], t.data(), sizeof(qeal) * t.size(), cudaMemcpyHostToDevice);

			t = host_sys_project_matrix_z[i];
			cudaMalloc((char**)&(dev_sys_project_matrix_z[i]), sizeof(qeal) * t.size());
			cudaMemcpy(dev_sys_project_matrix_z[i], t.data(), sizeof(qeal) * t.size(), cudaMemcpyHostToDevice);

			mMatrixX tm = host_sys_project_matrix_x[i].transpose();
			cudaMalloc((char**)&(dev_sys_project_matrix_t_x[i]), sizeof(qeal) * tm.size());
			cudaMemcpy(dev_sys_project_matrix_t_x[i], tm.data(), sizeof(qeal) * tm.size(), cudaMemcpyHostToDevice);

			tm = host_sys_project_matrix_y[i].transpose();
			cudaMalloc((char**)&(dev_sys_project_matrix_t_y[i]), sizeof(qeal) * tm.size());
			cudaMemcpy(dev_sys_project_matrix_t_y[i], tm.data(), sizeof(qeal) * tm.size(), cudaMemcpyHostToDevice);

			tm = host_sys_project_matrix_z[i].transpose();
			cudaMalloc((char**)&(dev_sys_project_matrix_t_z[i]), sizeof(qeal) * tm.size());
			cudaMemcpy(dev_sys_project_matrix_t_z[i], tm.data(), sizeof(qeal) * tm.size(), cudaMemcpyHostToDevice);
		}
	...
	void BendingCactusSimulator::freeCudaBuffer()
	{
	...
	void BendingCactusSimulator::run()
	{
		...
		cudaMemcpy(host_tet_nodes_position.data() + host_fullspace_dim_buffer_offset[sim_id], dev_tet_nodes_position + host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToHost);
	}

	void BendingCactusSimulator::oneIteration()
	{
		cudaMemcpy(dev_project_nodes_position + host_fullspace_dim_buffer_offset[sim_id], dev_Ms_n + host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToDevice);
		...
		cudaMemcpy(dev_tet_nodes_position +host_fullspace_dim_buffer_offset[sim_id], dev_tet_nodes_ori_position + +host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToDevice);
#ifdef USE_DOUBLE_PRECISION
		cublasDaxpy(blasHandle, host_total_fullspace_dim, &cublas_pos_one, dev_fullspace_displacement, 1, dev_tet_nodes_position, 1);
#else
		cublasSaxpy(blasHandle, host_fullspace_dim[sim_id], &cublas_pos_one, dev_fullspace_displacement + host_fullspace_dim_buffer_offset[sim_id], 1, dev_tet_nodes_position + host_fullspace_dim_buffer_offset[sim_id], 1);
#endif
	}

	void BendingCactusSimulator::calculateExternalForce()
	{
		cudaMemcpy(dev_tet_nodes_force + host_fullspace_dim_buffer_offset[sim_id], dev_tet_nodes_gravity_force + host_fullspace_dim_buffer_offset[sim_id], sizeof(qeal) * host_fullspace_dim[sim_id], cudaMemcpyDeviceToDevice);
...
#else
			cudaMemcpy(dev_tet_nodes_extra_force + host_fullspace_dim_buffer_offset[sim_id], host_tet_nodes_extra_force.data() + host_fullspace_dim_buffer_offset[sim_id], sizeof(qeal) *host_fullspace_dim[sim_id], cudaMemcpyHostToDevice);
			cublasSaxpy(blasHandle, host_fullspace_dim[sim_id], &cublas_pos_one, dev_tet_nodes_extra_force + host_fullspace_dim_buffer_offset[sim_id], 1, dev_tet_nodes_force + host_fullspace_dim_buffer_offset[sim_id], 1);
#endif
		}
	}

	void BendingCactusSimulator::calculateFirstItemRight()
	{
#ifdef USE_DOUBLE_PRECISION 
...
#else
		cublasScopy(blasHandle, host_fullspace_dim[sim_id], dev_fullspace_displacement + host_fullspace_dim_buffer_offset[sim_id], 1, dev_inertia_y + host_fullspace_dim_buffer_offset[sim_id], 1);

		cublasSaxpy(blasHandle, host_fullspace_dim[sim_id], &host_time_step, dev_tet_nodes_velocity + host_fullspace_dim_buffer_offset[sim_id], 1, dev_inertia_y + host_fullspace_dim_buffer_offset[sim_id], 1);
#endif

		cudaMemcpy(dev_tet_nodes_prev_position + host_fullspace_dim_buffer_offset[sim_id], dev_tet_nodes_position + host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToDevice);

		computeSnHost
		(
			host_fullspace_dim[sim_id],
			dev_fullspace_dim + sim_id,
			dev_tet_nodes_force + host_fullspace_dim_buffer_offset[sim_id],
			dev_mass_inv_vector+ host_fullspace_dim_buffer_offset[sim_id],
			dev_inertia_y + host_fullspace_dim_buffer_offset[sim_id]
		);
...
#else
		cublasSaxpy(blasHandle, host_fullspace_dim[sim_id], &cublas_pos_one, dev_inertia_y + host_fullspace_dim_buffer_offset[sim_id], 1, dev_tet_nodes_position + host_fullspace_dim_buffer_offset[sim_id], 1);
#endif
		cudaMemcpy(dev_Ms_n + host_fullspace_dim_buffer_offset[sim_id], dev_ori_b + host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToDevice);
		...
		(
			...
			dev_fullspace_dim + sim_id,
			dev_inertia_y + host_fullspace_dim_buffer_offset[sim_id],
			dev_mass_vector + host_fullspace_dim_buffer_offset[sim_id],
			dev_Ms_n + host_fullspace_dim_buffer_offset[sim_id]
		);

	}

	void BendingCactusSimulator::calculateVelocity()
	{
...
#else
		cudaMemcpy(dev_tet_nodes_velocity + host_fullspace_dim_buffer_offset[sim_id], dev_tet_nodes_position + host_fullspace_dim_buffer_offset[sim_id], host_fullspace_dim[sim_id] * sizeof(qeal), cudaMemcpyDeviceToDevice);
		cublasSaxpy(blasHandle, host_fullspace_dim[sim_id], &cublas_neg_one, dev_tet_nodes_prev_position + host_fullspace_dim_buffer_offset[sim_id], 1, dev_tet_nodes_velocity + host_fullspace_dim_buffer_offset[sim_id], 1);

		qeal scale = (1.0 - v_damp) / host_time_step;
		cublasSscal(blasHandle, host_fullspace_dim[sim_id], &scale, dev_tet_nodes_velocity + host_fullspace_dim_buffer_offset[sim_id], 1);
#endif
	}

	void BendingCactusSimulator::postRun()
	{
		updateSurface();
		updateMedialMesh();
	}
	
	void BendingCactusSimulator::updateSurface()
	{
		updateSurfaceHost
		(
	...
	void BendingCactusSimulator::updateMedialMesh()
	{
		...
		if (use_displacement_bounding || host_medial_cones_num[sim_id] == 1)
		{
			displacementBoundingHost
			(
	...
	void BendingCactusSimulator::initBoundingInfo()
	{
	...
	void BendingCactusSimulator::loadTextureBuffer()
	{
	...
	void BendingCactusSimulator::addMeshConfig(BendingCactusConfig * mc)
	{
	...
	mVector3 BendingCactusSimulator::getTetNodePosition(const uint32_t vid)
	{
		return mVector3(host_tet_nodes_position[3 * vid], host_tet_nodes_position[3 * vid + 1], host_tet_nodes_position[3 * vid + 2]);
	}

	mVector4i BendingCactusSimulator::getTetElementNodeIndex(const uint32_t eid)
	{
	...
	mVector3 BendingCactusSimulator::getSurfacePointPosition(const uint32_t vid)
	{
	...
	mVector3i BendingCactusSimulator::getSurfaceFaceIndex(const uint32_t fid)
	{
		return mVector3i(host_surface_faces_index[3 * fid], host_surface_faces_index[3 * fid + 1], host_surface_faces_index[3 * fid + 2]);
	}

	inline mVector4 BendingCactusSimulator::getMedialNodePosition(const uint32_t vid)
	{
	...
	bool BendingCactusConfig::loadSceneObjectiveConfig(const std::string path, TiXmlElement* item, const double timeStep)
	{
		time_step = timeStep;
	...
	bool BendingCactusConfig::loadMesh(const std::string path, const std::string name)
	{
		return  loadSurfaceMesh(path, name) && loadTetMesh(path, name) && loadMedialMesh(path, name);
	}

	bool BendingCactusConfig::loadSurfaceMesh(const std::string path, const std::string name)
	{
	...
	bool BendingCactusConfig::loadTetMesh(const std::string path, const std::string name)
	{
	...
	bool BendingCactusConfig::loadMedialMesh(const std::string path, const std::string name)
	{
	...
	int BendingCactusConfig::searchCloseTetNode(Eigen::Vector3d p)
	{
	...
	void BendingCactusConfig::computeSurfaceTetInterpolation()
	{
	...
	void BendingCactusConfig::computeMaTetInterpolation()
	{
	...
	bool BendingCactusConfig::isinsideTetElement(const uint32_t eid, const Eigen::Vector3d p, Eigen::Vector4d & weight)
	{
	...
	void BendingCactusConfig::computeBarycentricWeights(const uint32_t eid, const Eigen::Vector3d p, Eigen::Vector4d & weight)
	{
	...
	Eigen::Matrix4d BendingCactusConfig::computeTetElementInvShapeFunction(const uint32_t eid)
	{
		uint32_t tet[4];
	...
	void BendingCactusConfig::initlialization()
	{
		setFramesAndFixed();
	...
	void BendingCactusConfig::setFramesAndFixed()
	{
	...
	void BendingCactusConfig::computeMassAndGravity()
	{
	...
	void BendingCactusConfig::computeLaplacianMatrix(std::vector<Eigen::Triplet<double>>& matValue)
	{
	...
	void BendingCactusConfig::computeWeight(Eigen::MatrixXd & weight)
	{
	...
	void BendingCactusConfig::computeHarmonicProjectMatrix(Eigen::MatrixXd & pm)
	{
		uint32_t tn = tet_nodes_num;
	...
	void BendingCactusConfig::setTetStrainConstraints(Eigen::SparseMatrix<double>& Ytet)
	{
	...
	void BendingCactusConfig::computeSysMatrix()
	{
		...
		else
		{
			...
			cudaMemcpy(sys.data(), dev_d_sys, reduce_dim * reduce_dim * sizeof(double), cudaMemcpyDeviceToHost);
		}
	}

	void BendingCactusConfig::computeEncloseSpheres()
	{
	...
	void BendingCactusConfig::precomputeMedialPrimitiveBounding()
	{
	...
	void BendingCactusConfig::uniform()
	{
	...
	void BendingCactusConfig::computeSurfaceNormalAndBBox()
	{
	...
	void BendingCactusConfig::computeMedialPrimitiveBBox()
	{
	...
	Eigen::Vector3d BendingCactusConfig::getTetNodePosition(const uint32_t tid)
	{
		return Eigen::Vector3d(tet_nodes[3 * tid], tet_nodes[3 * tid + 1], tet_nodes[3 * tid + 2]);
	}

	Eigen::Vector3d BendingCactusConfig::getSurfacePointsPosition(const uint32_t sid)
	{
		return Eigen::Vector3d(surface_points[3 * sid], surface_points[3 * sid + 1], surface_points[3 * sid + 2]);
	}

	Eigen::Vector3d BendingCactusConfig::getMedialNodePosition(const uint32_t mid)
	{
		return Eigen::Vector3d(ma_nodes[4 * mid], ma_nodes[4 * mid + 1], ma_nodes[4 * mid + 2]);
	}

	double BendingCactusConfig::getMedialNodeRadius(const uint32_t mid)
	{
		return ma_nodes[4 * mid + 3];
	}

	inline double BendingCactusConfig::getNearestSphereOnMedialCone(const Eigen::Vector3d sc, const double sr, Eigen::Vector3d c11, double r11, Eigen::Vector3d c12, double r12, double & t)
	{
		bool inverse = false;
		if (r11 > r12)
		{
	...
	inline double BendingCactusConfig::getNearestSphereOnMedialSlab(const Eigen::Vector3d sc, const double sr, const Eigen::Vector3d c11, const double r11, const Eigen::Vector3d c12, const double r12, const Eigen::Vector3d c13, const double r13, double & t1, double & t2)
	{
		Eigen::Vector3d cq = sc;
		double rq = sr;
