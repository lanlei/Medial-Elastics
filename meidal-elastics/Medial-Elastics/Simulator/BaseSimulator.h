#pragma once
#ifndef BaseSimulator_H__
#define BaseSimulator_H__
...
#include "Common\EigenRename.h"
#include "Common\EigenMatrixIO.h"
#include "Common\StlContainerIO.h"
...
namespace MECR
{		
	struct BasicSimStatus
	{
		BasicSimStatus() :
			animationLifeTime(0),
			animationCost(0.0),
			startSimulation(false),
			renderMedialPrimitives(false)
		{}
		std::string path;
		uint32_t animationLifeTime;
		float animationCost;
		bool startSimulation;
		bool renderMedialPrimitives;
	};

	extern BasicSimStatus status;
	
	class ModelMeshConfig;

	class BaseSimulator
	{
	public:
		BaseSimulator():load_from_binary(false), host_objectives_num(0), host_time_step(10.0 / 1000.0), v_damp(0.0015), maxIter(10), host_cell_grid({-4, 0, -4, 4, 4, 4}),
		...
		void addMeshConfig(ModelMeshConfig* mc);
		...
		std::vector<uint32_t> host_detect_faces_self_collision_culling_flag;
		uint32_t* dev_detect_faces_self_collision_culling_flag;
	...
	protected:
		...
		bool isSurfaceBoundaryFaces(uint32_t fid);
		void searchSurfaceNeighborFaces(uint32_t fid, std::set<uint32_t>& setList, uint32_t ring = 1);
...
#define ENLARGE_RADIUS_COEFF 1.0
};
